...
async function searchAirports(query) {
  ...
  try {
    ...
    if (client) {
      ...
      if (cachedData) {
        console.log(
          "Returning cached data for airport search:",
          lowerCaseQuery,
        );
    ...
    const result = airportList.data.filter((airport) =>
      [airport.code, airport.name, airport.bandara].some((field) =>
...
async function cachedList() {
  try {
    ...
    if (client) {
      await client.set("airports_data", JSON.stringify(responseData), {
        EX: 24 * 60 * 60,
      });
    }
    return responseData;

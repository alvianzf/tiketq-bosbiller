...
app.use(
  cors({
    origin: function (origin, callback) {
      ...
      // Enable CORS headers ONLY for local development to avoid duplicates with Nginx on production
      if (origin.includes("localhost") || origin.includes("127.0.0.1")) {
        callback(null, true);
      } else {
        // Return false so Express CORS does not send Access-Control-Allow-Origin header
        callback(null, false);
      }
    },
    credentials: true,

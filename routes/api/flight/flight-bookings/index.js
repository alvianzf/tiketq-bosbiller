...
router.post("/", async (req, res) => {
  const { passenger_name, amount, flight_date, flight_carrier, book_date } =
    req.body;
  try {
    const newBooking = await FlightBookingDAO.createBooking(
      passenger_name,
      amount,
      flight_date,
      flight_carrier,
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Booking retrieved successfully", booking: booking });
  } catch (err) {
    console.error(err);
    res
      .status(500)
...
router.get("/bookNo/:bookNo", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: "Bookings by book number retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings by book number",
        message: "An error occurred while fetching bookings by book number.",
      });
  }
});
...
router.get("/name/:name", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: "Bookings by name retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings by name",
        message: "An error occurred while fetching bookings by name.",
      });
  }
});
  ...
  if (status !== "true" && status !== "false") {
    return res
      ...
      .json({
        ...
        message: "Please use true or false for payment status.",
      });
  }
  try {
    const paymentStatus = status === "true"; // Convert string to boolean
    const bookings =
      await FlightBookingDAO.findBookingsByPaymentStatus(paymentStatus);
    res
      .status(200)
      .json({
        message: "Bookings by payment status retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings by payment status",
        message: "An error occurred while fetching bookings by payment status.",
      });
  }
});
...
router.get("/airlines/:airlines", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: "Bookings by airlines retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings by airlines",
        message: "An error occurred while fetching bookings by airlines.",
      });
  }
});
...
router.get("/sortedByBookDate", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Bookings sorted by book date retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings sorted by book date",
        message:
          "An error occurred while fetching bookings sorted by book date.",
      });
  }
});
...
router.get("/sortedByFlightDate", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: "Bookings sorted by flight date retrieved successfully",
        bookings: bookings,
      });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({
        error: "Failed to retrieve bookings sorted by flight date",
        message:
          "An error occurred while fetching bookings sorted by flight date.",
      });
  }
});
